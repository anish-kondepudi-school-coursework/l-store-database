...
class Table:
    ...
    def __init__(self, name, num_columns, primary_key_col):
        ...
        self.rid_gen: RID_Generator = RID_Generator()
        self.page_ranges: list = [PageRange(self.num_columns, self.page_directory, self.rid_gen)]
    ...
    def insert_record(self, columns: list) -> bool:
        ...
        if last_page_range.is_full():
            new_page_range: PageRange = PageRange(self.num_columns, self.page_directory, self.rid_gen)
    ...
    def update_record(self, primary_key: int, columns: list) -> bool:
        rid: int = self.index.get_rid(primary_key)
        page_range_with_record: PageRange = self._find_page_range_with_rid(rid)
        self.index.delete_key(primary_key)
    ...
    def get_latest_column_values(self, primary_key: int, projected_columns_index: list):
        ...
        rid: int = self.index.get_rid(primary_key)
        page_range: PageRange = self._find_page_range_with_rid(rid)
        col_vals: list = []
    ...
    def _find_page_range_with_rid(self, rid: int):
        page_range_index: int = int(rid / NUM_RECORDS_IN_PAGE_RANGE)
